...
def authenticate(username, password):
    """
    Authenticate the user by checking the credentials in password.txt.
    Return the role (admin or student) if valid, otherwise None.
    """
    try:
        with open("data/passwords.txt", "r") as file: 
            for line in file:
                stored_username, stored_password, role = line.stripe().split(",")
...
def get_user_details(username):
    """
    Fetch user details from user.txt based on the username.
    ...
    try:
        ...
        print("error:users.txt file not founded.")
    except Exception as e:
        print(f"Error:{e}")
    return None 
